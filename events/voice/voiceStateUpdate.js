...
module.exports = class VoiceStateUpdate extends Event {
    ...
    async run(oldVoice, newVoice) {
        try {
            ...
            guildModel
                .findOne({ guildID: newVoice.guild.id })
                .then((s, err) => {
                    ...
                    if (
                        newVoice.channelId &&
                        newVoice.channelId == s.config.Pvc.StartingChannel
                    ) {
                        guild.channels
                            .create(userch.user.tag, {
                                type: 'GUILD_VOICE',
                            ...
                            .then(createdchannel => {
                                ...
                                s.config.Pvc.TemporaryChannels.push(
                                    createdchannel.id + newVoice.id
                                )
            ...
            if (!player) return
            if (!this.client || !this.client.user || !this.client.user.id)
                return
            ...
            if (
                ...
                if (
                    oldVoice.guild.me.voice.channel &&
                    oldVoice.guild.me.voice.channel.members &&
                    oldVoice.guild.me.voice.channel.members.size === 1
                ) {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setDescription(
                            `${
                                this.client.language.VOICESTATEUPDATE[1]
                            } **${vcName}** ${
                                this.client.language.VOICESTATEUPDATE[2]
                            } ${300000 / 1000} ${
                                this.client.language.VOICESTATEUPDATE[3]
                            }`
                        )
                    const channel2 = await this.client.channels.cache.get(
                        player.textChannel
                    )
                    if (!channel2) return
                    ...
                    const vcMembers =
                        oldVoice.guild.me.voice.channel.members.size
                    if (!vcMembers || vcMembers === 1) {
                        ...
                        const embed2 = new MessageEmbed()
                            .setColor(process.env.EMBED_COLOR)
                            .setDescription(
                                this.client.language.VOICESTATEUPDATE[4]
                            )
