    ...
    async run(oldVoice, newVoice) {
        try {
                        ...
                        s.config.Pvc.TemporaryChannels.forEach((canal) => {
                            ...
                            if (oldVoice.channelId == canalID) {
                                s.config.Pvc.TemporaryChannels.splice(
                                    s.config.Pvc.TemporaryChannels.indexOf(channel2.id + newVoice.id),
                                    s.config.Pvc.TemporaryChannels.indexOf(channel2.id + newVoice.id) + 1
                                )
                                channel2.delete()
            ...
            if (
                ...
                oldVoice.guild.members.cache.get(this.client.user.id).voice.channel.id == oldVoice.channelId
            ) {
                if (
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setDescription(
                            `${this.client.language.VOICESTATEUPDATE[1]} **${vcName}** ${
