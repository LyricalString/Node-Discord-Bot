...
module.exports = class Wink extends Command {
  constructor(client) {
    super(client, {
      name: "wink",
      description: [
        "Winks to the mentioned user.",
        "Guiña el ojo al usuario mencionado.",
      ],
      usage: ["[@user]", "[@usuario]"],
    ...
    try {
      ...
      if (args[0]) {
          ...
          let embed = new Discord.MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
            .setTitle(`${author} ${client.language.WINK[4]}`)
      ...
      if (!user) {
        ...
        let author = message.author.username;
        let embed = new Discord.MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
          .setTitle(`${author} ${client.language.WINK[3]} ${args.join(" ")}`)
          .setColor(process.env.EMBED_COLOR)
          .setImage(soyultro("wink"));
        return message.channel.send({embeds: [embed]});
      }
      ...
      let author = message.author.username;
      let embed = new Discord.MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
        .setTitle(`${author} ${client.language.WINK[3]} ${user.user.username}`)
        .setColor(process.env.EMBED_COLOR)
        .setImage(soyultro("wink"));
