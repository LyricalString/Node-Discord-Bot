...
module.exports = class Wave extends Command {
    constructor(client) {
        super(client, {
            name: 'wave',
            description: [
                'Waves the hand to the mentioned user.',
                'Saluda con la mano al usuario mencionado.'
            ],
            usage: ['[@user]', '[@usuario]'],
    ...
    async run(client, message, args, prefix, lang, webhookClient, ipc) {
        try {
            ...
            if (!user) {
                ...
                let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                    .setTitle(
                        `${author} ${client.language.WAVE[3]} ${args.join(' ')}`
                    )
                    .setColor(process.env.EMBED_COLOR)
                    .setImage(soyultro('wave'))
                return message.channel.send({ embeds: [embed] })
            ...
            let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                .setTitle(
                    `${author} ${client.language.WAVE[3]} ${user.user.username}`
                )
                .setColor(process.env.EMBED_COLOR)
                .setImage(soyultro('wave'))
