...
module.exports = class Bye extends Command {
    constructor(client) {
        super(client, {
            name: 'bye',
            description: [
                'Says goodbye to the mentioned user.',
                'Le dice adios al usuario mencionado.'
            ],
            usage: ['[@user]', '[@usuario]'],
    ...
    async run(client, message, args, prefix, lang, webhookClient, ipc) {
        try {
            ...
            if (!user) {
                ...
                let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                    .setTitle(
                        `${author} ${client.language.BYE[3]} ${args.join(' ')}`
                    )
                    .setColor(process.env.EMBED_COLOR)
                    .setImage(soyultro('bye'))
                return message.channel.send({ embeds: [embed] })
            ...
            let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                .setTitle(
                    `${author} ${client.language.BYE[3]} ${user.user.username}`
                )
                .setColor(process.env.EMBED_COLOR)
                .setImage(soyultro('bye'))
