...
module.exports = class Bully extends Command {
    constructor(client) {
        super(client, {
            name: 'bully',
            description: [
                'Bullies to the mentioned user.',
                'Hace bullying al usuario mencionado.'
            ],
            usage: ['<@user>', '<@usuario>'],
            tos: true,
        ...
        try {
            ...
            if (!user) {
                ...
                let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                    .setTitle(
                        `${author} ${client.language.BULLY[3]} ${args.join(
                            ' '
                        )}`
                    )
            ...
            let embed = new MessageEmbed() //Preferible mandarlo en un Embed ya que la respuesta es un link
                .setTitle(
                    `${author} ${client.language.BULLY[3]} ${user.user.username}`
                )
                .setColor(process.env.EMBED_COLOR)
                .setImage(soyultro('bully'))
            if (args.length > 1) {
